...
def shadowingfunction_20_updated(a, vegdem, vegdem2, azimuth, altitude, scale, amaxvalue, bush, forsvf):
    ...
    while amaxvalue >= dzprev and np.abs(dx) < sizex and np.abs(dy) < sizey:
        ...
        xc1, xc2 = max(0, int((dx + absdx) / 2.)), min(sizex, int(sizex + (dx - absdx) / 2.))
        yc1, yc2 = max(0, int((dy + absdy) / 2.)), min(sizey, int(sizey + (dy - absdy) / 2.))
        xp1, xp2 = max(0, int(-((dx - absdx) / 2.))), min(sizex, int(sizex - (dx + absdx) / 2.))
        yp1, yp2 = max(0, int(-((dy - absdy) / 2.))), min(sizey, int(sizey - (dy + absdy) / 2.))
        tempvegdem = vegdem[xc1:xc2, yc1:yc2] - dz
        tempvegdem2 = vegdem2[xc1:xc2, yc1:yc2] - dz
        a_slice = a[xp1:xp2, yp1:yp2]
    
        # Ensure shapes match for broadcasting
        min_shape = (min(tempvegdem.shape[0], a_slice.shape[0]), min(tempvegdem.shape[1], a_slice.shape[1]))
        tempvegdem = tempvegdem[:min_shape[0], :min_shape[1]]
        a_slice = a_slice[:min_shape[0], :min_shape[1]]

        f = np.fmax(f[:min_shape[0], :min_shape[1]], tempvegdem)  # Moving building shadow
        sh[:min_shape[0], :min_shape[1]] = np.where(f > a_slice, 1, 0).astype(np.float32)
        
        fabovea = tempvegdem > a_slice
        vbshvegsh[:min_shape[0], :min_shape[1]] = np.where(fabovea, 1, 0).astype(np.float32)
        vegsh[:min_shape[0], :min_shape[1]] = np.fmax(vegsh[:min_shape[0], :min_shape[1]], vbshvegsh[:min_shape[0], :min_shape[1]])

        dzprev = dz
        index += 1

    return {'vegsh': vegsh, 'sh': sh}


def shadowingfunction_20_updated_first(a, vegdem, vegdem2, azimuth, altitude, scale, amaxvalue, bush, forsvf):
